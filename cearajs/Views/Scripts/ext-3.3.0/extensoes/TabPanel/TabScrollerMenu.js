		...
		generateTabMenuItems : function() {
			...
			if (totalItems > pageSize)  {
				...
				for (var i = 0 ; i < numSubMenus; i++) {
					...
					this.tabsMenu.add({
						text : this.tabScrollerMenu.getMenuPrefixText() + ' '  + (curPage - pageSize + 1) + ' &agrave ' + curPage,
						menu : menuItems
					});
				...
				if (remainder > 0) {
					...
					this.tabsMenu.add({
						text : this.tabScrollerMenu.menuPrefixText  + ' ' + (start + 1) + ' &agrave ' + (start + menuItems.length),
						menu : menuItems
					});
...
Ext.preg('tabscrollermenu', Ext.ux.TabScrollerMenu);
